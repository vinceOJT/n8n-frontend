...
import { User, Search, Menu, ArrowLeft } from "lucide-react"
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      if (response.ok) {
        ...
        setHtmlContent(htmlResult); // Store the n8n HTML in state
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-white selection:bg-[#FFD200] selection:text-black">
      ...
      <main className="min-h-screen flex items-center justify-center pt-20 p-4 md:p-12 relative overflow-hidden">
        ...
        {/* Conditional Rendering: Show result if htmlContent exists, otherwise show form */}
        {!htmlContent ? (
          ...
          /* Blog Result View */
          <div className="max-w-4xl w-full bg-white rounded-[2.5rem] p-4 md:p-8 shadow-2xl relative z-10 animate-in fade-in zoom-in duration-300">
            <Button
              variant="ghost"
            ...
            {/* Render the n8n HTML content */}
            <div
              className="overflow-auto max-h-[70vh] rounded-2xl border border-slate-100"
